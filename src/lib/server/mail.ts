...
export async function sendMailMessage(
	email: string,
	message: string,
	subject: string = 'Important Notification from MediSYNC'
) {
	const newMail = new MailService()
		.from({ email: env.SMTP2GO_SENDER_EMAIL, name: 'MediSYNC Health Office' })
		.to({ email })
		.subject(subject).html(`
			<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
				<div style="background-color: #f8f9fa; padding: 20px; text-align: center; border-bottom: 1px solid #dee2e6;">
					<h2 style="color: #2c3e50; margin: 0;">MediSYNC Health Office</h2>
				</div>
				...
				<div style="background-color: #f8f9fa; padding: 15px; text-align: center; border-top: 1px solid #dee2e6; font-size: 12px; color: #6c757d;">
					<p style="margin: 0;">This message was sent from MediSYNC School Health Management System.</p>
